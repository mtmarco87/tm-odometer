...
TmOdometer.options = window.odometerOptions ? {}
...
TmOdometer.init = ->
  ...
  elements = document.querySelectorAll (TmOdometer.options.selector or '.odometer')

  for el in elements
    el.odometer = new TmOdometer {el, value: (el.innerText ? el.textContent)}

if document.documentElement?.doScroll? and document.createEventObject?
  # IE < 9
  _old = document.onreadystatechange
  document.onreadystatechange = ->
    if document.readyState is 'complete' and TmOdometer.options.auto isnt false
      TmOdometer.init()

    _old?.apply this, arguments
else
  document.addEventListener 'DOMContentLoaded', ->
    if TmOdometer.options.auto isnt false
      TmOdometer.init()
  , false
